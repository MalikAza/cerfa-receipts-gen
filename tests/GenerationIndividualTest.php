...
final class GenerationIndividualTest extends TestCase {
    private static array $testData;
    private static string $tmpPath;

    public static function setUpBeforeClass(): void {
        $content = file_get_contents(__DIR__ . '/json/cerfa.test.json');
        self::$testData = json_decode($content, true)['generateIndividual'];
    ...
    public function testGenerateText(): void {
        ...
        foreach ($testData as $key => $value) {
            $templatePath = Config::get('CERFA_INDIVIDUAL_PATH');
            $data = [$key => $value];
